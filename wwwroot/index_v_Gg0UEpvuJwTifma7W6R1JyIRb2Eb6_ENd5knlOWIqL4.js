function fuzzysearch(n,t){var r=t.length,u=n.length,i,f,e;if(u>r)return!1;if(u===r)return n===t;n:for(i=0,f=0;i<u;i++){for(e=n.charCodeAt(i);f<r;)if(t.charCodeAt(f++)===e)continue n;return!1}return!0}function formatDuration(n){if(isNaN(n))return"0:00";n<0&&(n=-n);const t={hour:Math.floor(n/3600)%24,minute:Math.floor(n/60)%60,second:Math.floor(n)%60};return Object.entries(t).filter((n,t)=>t||n[1]).map(n=>(n[1]+"").padStart(2,"0")).join(":")}function writeText(n){const t=document.createElement("textarea");t.style.position="fixed";t.style.right="100%";document.body.appendChild(t);t.value=n;t.select();document.execCommand("copy");t.remove()}async function getBaseAddress(){const n=await fetch("/api/video/ck");return n.text()}function hmsToSecondsOnly(n){for(var t=n.split(":"),i=0,r=1;t.length>0;)i+=r*parseInt(t.pop(),10),r*=60;return i}function initializeMenu(){window.menu.attach(document.querySelector(".menu"));window.menu.eventHandler=async(n,t,i)=>{if(n===1)t.startsWith("http://")||t.startsWith("https://")?writeText(t):writeText(baseUri+t);else if(n===2)try{const n=await fetch(`/api/video?id=${t}`,{method:"DELETE"});toast.setAttribute("message","Success.")}catch(r){}else n===3?t.startsWith("http://")||t.startsWith("https://")?window.JInterface.download(t,i.querySelector("h3").textContent):window.JInterface.download(baseUri+t,i.querySelector("h3").textContent):n===5&&writeText(t.querySelector("h3").textContent)}}function onItemClick(n){n.addEventListener("click",async()=>{const t=n.getAttribute("data-href"),i=n.getAttribute("data-id");window.JInterface&&(t.startsWith("http://")||t.startsWith("https://")?window.JInterface.handleRequest(t,i):window.JInterface.handleRequest(baseUri+t,i))})}function onContextItem(n){n.addEventListener("click",n=>{n.stopPropagation();let t=n.currentTarget;while(t&&t.className!=="large-media-item")t=t.parentNode;window.menu.show(t)})}async function initialize(){baseUri=await getBaseAddress();initializeMenu()}function render(n){const t=document.createDocumentFragment();for(let i=0;i<n.length;i++){const r=n[i];t.appendChild(makeItem(r))}container.appendChild(t)}function makeItem(n){const t=document.createElement("DIV");t.setAttribute("class","large-media-item");t.setAttribute("data-id",n.id);t.setAttribute("data-href",n.url);const w=document.createElement("A"),u=document.createElement("DIV");u.setAttribute("class","video-thumbnail-container-large");const b=document.createElement("DIV");b.setAttribute("class","video-thumbnail-bg cover");u.appendChild(b);const s=document.createElement("IMG");s.setAttribute("class","cover");s.setAttribute("src",n.thumbnail);u.appendChild(s);const h=document.createElement("DIV");h.setAttribute("class","video-thumbnail-overlay-bottom-group");const c=document.createElement("DIV");c.setAttribute("class","thumbnail-overlay-time-status-renderer");c.appendChild(document.createTextNode(formatDuration(n.duration)));h.appendChild(c);u.appendChild(h);w.appendChild(u);t.appendChild(w);const f=document.createElement("DIV");f.setAttribute("class","details");const l=document.createElement("DIV");l.setAttribute("class","large-media-channel");const i=document.createElement("DIV");i.setAttribute("class","channel-thumbnail-icon");i.style.fontSize="8px!important";i.style.display="flex";i.style.alignItems="center";i.style.justifyContent="center";i.appendChild(document.createTextNode(n.type===3?"57":n.type===1?"91":"XV"));l.appendChild(i);f.appendChild(l);const e=document.createElement("DIV");e.setAttribute("class","large-media-item-info");const a=document.createElement("DIV");a.setAttribute("class","large-media-item-metadata");const k=document.createElement("A"),v=document.createElement("H3");v.setAttribute("class","large-media-item-headline");v.appendChild(document.createTextNode(n.title));k.appendChild(v);a.appendChild(k);e.appendChild(a);const o=document.createElement("DIV");o.setAttribute("class","large-media-item-menu");const y=document.createElement("BUTTON");y.setAttribute("class","icon-button");const p=document.createElement("DIV");p.setAttribute("class","c3-icon");const r=document.createElementNS("http://www.w3.org/2000/svg","svg");r.setAttribute("xmlns","http://www.w3.org/2000/svg");r.setAttribute("enable-background","new 0 0 24 24");r.setAttribute("height","24");r.setAttribute("viewBox","0 0 24 24");r.setAttribute("width","24");const d=document.createElementNS("http://www.w3.org/2000/svg","path");return d.setAttribute("d","M12,16.5c0.83,0,1.5,0.67,1.5,1.5s-0.67,1.5-1.5,1.5s-1.5-0.67-1.5-1.5S11.17,16.5,12,16.5z M10.5,12 c0,0.83,0.67,1.5,1.5,1.5s1.5-0.67,1.5-1.5s-0.67-1.5-1.5-1.5S10.5,11.17,10.5,12z M10.5,6c0,0.83,0.67,1.5,1.5,1.5 s1.5-0.67,1.5-1.5S12.83,4.5,12,4.5S10.5,5.17,10.5,6z"),r.appendChild(d),p.appendChild(r),y.appendChild(p),o.appendChild(y),e.appendChild(o),f.appendChild(e),t.appendChild(f),onItemClick(t),onContextItem(o),t}async function loadVideos(n,t){/\d+:\d+/.test(n)&&(n="tj:"+hmsToSecondsOnly(n));const r=await fetch(n?`/api/video/query?keyword=${n}&factor=${t}&region=${region}`:`/api/video?count=40&factor=${t}&order=${order}&region=${region}`),i=await r.json();i.length<20&&stop();const u=n==="*"||n==="91"||n==="-91"||n==="51"||n==="1"||!n||n.startsWith("tj:")?i:i.filter(t=>fuzzysearch(n,t.title));render(u)}function stop(){stopped=!0;spinner.style.display="none";toast.setAttribute("message","已全部加载")}async function tryUpdateVideos(){const n=await fetch("/api/video/57ck");if(!n.ok)throw new Error(n.statusText);return await n.text()}(function(){class n{constructor(){}inject(n){const t=document.createElement("template");t.innerHTML=`
<style>
@keyframes spinner {
            0% {
    ...
    <div class="menu-container">
      ...
      <c3-overlay>
        <button class="hidden-button" aria-label="close">
            ...
            `;document.body.appendChild(t);const i=n.attachShadow({mode:"open"});i.appendChild(t.content.cloneNode(!0));this.root=i;document.body.appendChild(n);this.initialize()}initialize(){const n=this.root.querySelector(".menu-container");n.style.display="none";this.root.querySelector("c3-overlay").addEventListener("click",()=>{n.style.display="none"});this.menuContainer=n;const i=this.root.querySelector("#action-copy");i.addEventListener("click",()=>{const t=this.element.getAttribute("data-href");n.style.display="none";this.callback&&this.callback(1,t)});const r=this.root.querySelector("#action-copy-title");r.addEventListener("click",()=>{n.style.display="none",this.callback&&this.callback(5,this.element)});const t=this.root.querySelector("#action-delete");document.cookie?t.addEventListener("click",()=>{const t=this.element.getAttribute("data-id");n.style.display="none";this.callback&&this.callback(2,t)}):t.setAttribute("hidden","");const u=this.root.querySelector("#action-download");u.addEventListener("click",()=>{const t=this.element.getAttribute("data-href");n.style.display="none";this.callback&&this.callback(3,t,this.element)})}hide(){}show(n){this.menuContainer.removeAttribute("style");this.element=n}}window.menu=new n}();let baseUri;const toast=document.querySelector("custom-toast");initialize().then(()=>{}).catch(()=>{toast.setAttribute("message","无法加载页面")});const container=document.querySelector(".container"),spinner=document.querySelector(".spinner");window.spinner.inject(spinner);let keyword,factor=0,stopped=!1,order=2,region=0,loading=!1;const observer=new IntersectionObserver(async n=>{if(n[0].isIntersecting&&!loading){if(stopped)return;try{loading=!0;await loadVideos(keyword,factor);factor++}catch(t){stop()}loading=!1}});observer.observe(spinner);let keywords;if(window.localStorage){const n=window.localStorage.getItem("keywords");keywords=n?JSON.parse(n):[];keywords.length>8&&(keywords=keywords.slice(0,8));window.header.appendKeywords(keywords)}window.header.placeholder="搜索";window.header.submit=async n=>{window.header.hide(),spinner.removeAttribute("style"),keyword=n,factor=0,stopped=!1,container.innerHTML="",keyword&&keywords&&(keywords.length>8&&keywords.pop(),keywords.unshift(n),keywords=[...new Set(keywords)],window.localStorage.setItem("keywords",JSON.stringify(keywords)),window.header.appendKeywords(keywords))};window.filter.callback=async n=>{factor=0,stopped=!1,order=n,container.innerHTML=""};window.filter.regionCallback=async()=>{factor=0,stopped=!1,region=3,container.innerHTML="",await tryUpdateVideos()}